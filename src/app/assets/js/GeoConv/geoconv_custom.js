...
(function geoconvModule (){
	...
	api.prototype.get_ellipsoid_data = function get_ellipsoid_data(eId) {/*{{{*/
		...
		// Load basic data:
		eId = (eId === undefined) ? this.e : this.check_ellipsoid(eId); // Use current if unspecified.
		var e = ellipsoids[eId];
		var name = e[0];
		var date = e[1];
		var a = e[2]; // Semieje mayor.
	...
	api.prototype.geo2utm = function geo2utm(fiDeg, LambdaDeg, e, aTz) { // Returns [x, y, TimeZone] /*{{{*/
		...
		// Otros cálculos:/*{{{*/
		var tz = (typeof aTz != 'undefined') ?aTz :trunc((LambdaDeg/6)+31); // TimeZone calculation.
	...
	api.prototype.utm2geo_deg = function utm2geo_deg(x, y, tz, NS, e) {//{{{
		...
		return [
			[lpad(LatD) + "&#186;" + lpad(LatM) + "&#39;" + lpad(LatS) + "&#39;" + "&#39;"] ,
			[lpad(LongD) + "&#186;" + lpad(LongM) + "&#39;" + lpad(LongS) + "&#39;" + "&#39;"]
		];

	}//}}}

	function lpad(aNumber) {
		var intNumber = parseInt(aNumber);
